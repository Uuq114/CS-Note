...
目前主流大模型的都是 decoder-only 结构，而非最早提出的 encoder-decoder 结构：

- decoder-only：把所有内容看成一个连续序列，即 “续写”
- encoder-decoder：明确区分源序列和目标序列，把任务建模成 “先理解一个输入，再生成一个序列”

1. **Why Scaling Up：为什么要扩大规模 **

   扩大模型参数量、训练数据和计算量，通常能够稳定降低预训练 loss，并改善下游任务表现。相比结果不确定的算法探索，Scaling 的收益更容易预测，也能通过知识蒸馏转移到更小的部署模型。因此，Scaling 不只是追求排行榜成绩，更是一种风险相对可控的工程和商业投资。

2. **Which Language Model to Scale Up：扩展哪种语言模型 **

   Slides 比较了 decoder-only、encoder-decoder 等架构，以及 autoregressive、Prefix LM、Masked LM 等训练目标。实验表明，decoder-only + autoregressive LM 在预训练后直接进行 zero-shot 时表现更好，而且训练信号密集、收敛较快、稳定性较高，因此成为当前通用 LLM 的主流方案。不过，多任务微调后 encoder-decoder + MLM 可能更优，说明模型选择仍取决于最终使用场景。

3. **What Factors Matter in Scaling：哪些因素决定 Scaling 效果 **

   最关键的三个因素是模型参数量 N、训练数据量 D 和计算量 C（预训练阶段消耗的计算量，单位 FLOPs）。在模型架构、数据分布和训练方法基本不变时，语言模型 loss 与这三个因素呈稳定的经验幂律关系。网络宽深比、FFN 比例等结构细节在合理范围内影响相对较小。Scaling 的收益虽然可预测，但边际收益会不断递减，因此模型、数据和计算必须协调增长，不能只扩大其中一个因素。

4. **What Configurations to Scale Up：如何选择扩展配置 **

   固定计算预算下，核心问题不是单纯选择最大的模型，而是确定模型参数量与训练 token 数之间的最优配比。标准方法是先进行多组小规模实验，拟合适用于当前模型和数据的 scaling law，再外推最终的大规模训练配置。Chinchilla 以 70B 参数和 1.4T tokens，在近似相同 FLOPs 下超过了 280B 参数、仅训练 300B tokens 的 Gopher，说明更小但训练更充分的模型可能更优。具体 scaling function 会随数据、架构和训练配方变化，不存在通用公式。

5. **Capabilities Emerged from Scaling Up：规模扩大后出现的能力 **

   随着规模扩大，预训练 loss 通常平滑下降，但 zero-shot 解题、推理、指令遵循和工具使用等能力可能在某个规模后突然达到可用水平，这被称为 “涌现能力”。部分突变可能来自 Exact Match 等离散评测指标，因为底层能力使用连续指标观察时可能仍是平滑增长的；但真实用户价值本身也具有阈值，例如代码必须能够运行、答案必须完全正确。因此，涌现既可能是评测造成的表象，也可能反映真实的可用性变化。同时，这些能力往往只能在极大规模下观察，使相关研究和知识逐渐集中到少数拥有巨额算力的机构。
